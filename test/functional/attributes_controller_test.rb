...
class AttributesControllerTest < Test::Unit::TestCase
  ...
  def test_show
    ...
    assert_not_nil assigns(:attribute)
    assert assigns(:attribute).valid?
  end
  ...
  def test_create
    num_attributes = Attribute.count
    ...
    assert_equal num_attributes + 1, Attribute.count
  end
  ...
  def test_destroy
    assert_not_nil Attribute.find(1)
